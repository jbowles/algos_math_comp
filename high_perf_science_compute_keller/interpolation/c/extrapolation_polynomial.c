...
result interpolate(float *xs, float *ys, int n, int order)
{
    ...
    for (int j = 0; j < 10; j++)
    {
        res.xs[(BASE_RES - 1) * 10 + j] = xs[BASE_RES - 1] + 0.1 * ((float)j) * delta;
        res.ys[(BASE_RES - 1) * 10 + j] = interp(xs + n - order, ys + n - order, res.xs[(BASE_RES - 1) * 10 + j], order);
...
//gcc extrapolation_polynomial.c -lm -o extrap
// ./extrap > extrap_out
int main()
{
